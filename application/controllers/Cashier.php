...
class Cashier extends CI_Controller {
    ...
    public function index()
    {
        ...
        if($this->input->post('param') == 'additem' and $this->input->post('barcode_id') != ""){
            $result = $this->amrta_model->select_query('vbarcode', array('barcode_id' => $this->input->post('barcode_id')), 'SINGLE');
            if($result['SOLD'] == "N") {
                $this->amrta_model->insert_query('tblcashier', array('barcode_id' => $this->input->post('barcode_id')));    
            } else {
                $this->session->set_flashdata('error_message', 'Barang tidak ditemukan atau sudah terjual');
                header('Location: '.site_url().'/cashier');
            }
        }
        
        $data['cart'] = $this->amrta_model->select_query('vcashier');
        $data['cart_sum'] = $this->amrta_model->select_query('vcashier_sum', FALSE, 'SINGLE');
        $data['mainmenu'] = $this->load->view('vMainMenu', '', TRUE);
        $this->load->view('cashier/vCashierMain', $data);
    }
    ...
    public function checkout()
    {
        ...
        if($this->input->post('pay_amount') != NULL){
            $total = $this->amrta_model->select_query('vcashier_sum', FALSE, 'SINGLE');
            
            // insufficient payment amount
            if($this->input->post('pay_amount') < $total['item_price']) {
                $this->session->set_flashdata('error_message', 'Pembayaran kurang');
            } else {
                // insert tblpurchase
                $purchase_id = date("YmdHis");
                $data = array(
                    'purchase_id'   => $purchase_id,
                    'customer_phone'=> $this->input->cookie('member_hp'),
                    'payment_amount'=> $this->input->post('pay_amount')
                );
                $this->amrta_model->insert_query('tblpurchase', $data);
                
                // insert tblpurchdetail
                $barcode = $this->amrta_model->select_query('vcashier');
                foreach($barcode as $bc){
                    $data = array(
                        'purchase_id'       => $purchase_id,
                        'barcode_id'        => $bc['barcode_id'],
                        'item_ori_price'    => $bc['item_ori_price'],
                        'item_price'        => $bc['item_price']
                    );
                    $this->amrta_model->insert_query('tblpurchdetail', $data);
                }
                
                // clear tblcashier
                $this->amrta_model->empty_table('tblcashier');
                
                
                // print the receipt
                $connector = new WindowsPrintConnector('POS58');
                $printer = new Escpos($connector);
                $logo = new EscposImage(base_url()."images/amrta-receipt.png");

                $sum = $this->amrta_model->select_query('vpurchase', array('purchase_id' => $purchase_id), 'SINGLE');
                $list = $this->amrta_model->select_query('vpurchdetail', array('purchase_id' => $purchase_id));

                $printer->setJustification(Escpos::JUSTIFY_CENTER);
                $printer->bitImage($logo);
                $printer->feed();
                $printer->text("==============================\n");
                $printer->text(date('l, j M Y')."\n");
                $printer->text("Untung Surapati - Amlapura\n");
                $printer->text("TranID : ".$purchase_id."\n\n\n");

                foreach($list as $item) {
                    $printer->setJustification(Escpos::JUSTIFY_LEFT);
                    $printer->text($item['item_name']."\n");
                    $printer->setJustification(Escpos::JUSTIFY_RIGHT);
                    $printer->text(number_format($item['item_price'])."\n");
                }
                ...
                $printer->text("Total\n");
                $printer->setJustification(Escpos::JUSTIFY_RIGHT);
                $printer->text(number_format($sum['tran_amount'])."\n");
                $printer->selectPrintMode();

                $printer->setJustification(Escpos::JUSTIFY_LEFT);
                $printer->text("Bayar\n");
                $printer->setJustification(Escpos::JUSTIFY_RIGHT);
                $printer->text(number_format($sum['payment_amount'])."\n");

                $printer->setJustification(Escpos::JUSTIFY_LEFT);
                $printer->text("Kembali\n");
                $printer->setJustification(Escpos::JUSTIFY_RIGHT);
                $printer->text(number_format($sum['payment_amount']-$sum['tran_amount'])."\n");

                $printer -> feed(2);
                $printer -> setJustification(Escpos::JUSTIFY_CENTER);
                $printer -> text("Matur Suksma\nTerima Kasih - Thank You\n");
                $printer -> feed(4);
                ...
                $this->session->set_flashdata('success_message', 'Pembayaran berhasil. Kembali Rp.'.number_format($this->input->post('pay_amount')-$total['item_price']) );
            }
        }
        header('Location: '.site_url().'/cashier');
    ...
    public function trans($purchase_id = FALSE)
    {
        $data['mainmenu'] = $this->load->view('vMainMenu', '', TRUE);
        if($this->session->flashdata('purchase_id') != "") {
            $data['info'] = $this->amrta_model->select_query('vpurchase', array('purchase_id' => $this->session->flashdata('purchase_id')), 'SINGLE');
            $data['trans'] = $this->amrta_model->select_query('vpurchdetail', array('purchase_id' => $this->session->flashdata('purchase_id')));
            $this->load->view('cashier/vTransDetail', $data);
        } else if($this->input->post('date_start') != '') {
            $wr = array(
                'purchase_date >=' => date('Y-m-d', strtotime($this->input->post('date_start'))),
                'purchase_date <=' => date('Y-m-d', strtotime($this->input->post('date_end')))
            );
            $data['trans'] = $this->amrta_model->select_query('vpurchase', $wr);
            $this->load->view('cashier/vTrans', $data);   
        } else if($purchase_id == FALSE) {
            //get today transaction
            $data['trans'] = $this->amrta_model->select_query('vpurchase', array('purchase_date >=' => date('Y-m-d')));
            $this->load->view('cashier/vTrans', $data);    
        } else if($purchase_id != FALSE) {
    ...
    public function discount($tp = FALSE)
    {
        $data['mainmenu'] = $this->load->view('vMainMenu', '', TRUE);
        if($this->session->userdata('user') != ""){
        
            if($this->input->post('purchase_id') != "" and $this->input->post('barcode') == "") {
                $data['info'] = $this->amrta_model->select_query('vpurchase', array('purchase_id' => $this->input->post('purchase_id')), 'SINGLE');
                $data['trans'] = $this->amrta_model->select_query('vpurchdetail', array('purchase_id' => $this->input->post('purchase_id')));
                $this->load->view('cashier/vTransDetailUpd', $data);
            } else if ($tp == "UPDATE") {
                $barcodes = $this->input->post('barcode');
                $prices = $this->input->post('price');
                $initials = $this->input->post('initialprice');
                ...
                for($i = 0; $i < count($barcodes); $i++){
                    ...
                    $this->amrta_model->update_query('tblpurchdetail', array('item_price' => $prices[$i]), $wr);

                    $data = array(
                        'purchase_id'   => $this->input->post('purchase_id'),
                        'barcode_id'    => $barcodes[$i],
                        'initial_price' => $initials[$i],
                        'final_price'   => $prices[$i],
                        'username'      => $this->session->userdata('user')
                    );
                    $this->amrta_model->insert_query('tblpurchaudit', $data);
                }
                header('Location: '.site_url().'/cashier/trans');
            } else {
                header('Location: '.site_url().'/cashier/trans');
